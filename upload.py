...
def upload_file(service, file_to_upload, parent_folder_id: str):
    ...
    if not path_to_file.exists():
        raise FileNotFoundError(
            f"{path_to_file} doesn't exist. Check if you provided the correct path."
        )
...
def upload_walk(local_dir: str, service, upload_folder_id):
    ...
    for root, dirs, files in walk_itr:
        ...
        for dir in tqdm(dirs, desc="Creating folders", leave=False):
            folder_id_memo[os.path.join(root, dir)] = create_folder(
                service, dir, folder_id_memo.get(root, upload_folder_id)
            )
        ...
        for file in tqdm(files, desc="Uploading files", leave=False):
            upload_file(
                ...
                folder_id_memo.get(root, upload_folder_id),
            )
...
def main(*args):
    ...
    upload_file(service, file_to_upload, upload_folder_id)
